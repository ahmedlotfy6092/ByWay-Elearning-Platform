...
export function EditInstructorDialog({
  ...
  const [jobTitle, setJobTitle] = useState(instructor.jobTitle);
  const [description, setDescription] = useState(instructor.description);
  const [rate, setRate] = useState(instructor.rate);
  const [image, setImage] = useState(instructor.image);
  const [previewUrl, setPreviewUrl] = useState(instructor.image);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update instructor information and profile details
          </DialogDescription>
        </DialogHeader>
